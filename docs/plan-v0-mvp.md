...
- `/init-brain` skill (manually create `.brain-config.yaml`)
...
- Separate `project-brain` git repo — skills live in this repo's `.claude/skills/`
...
### Files created in this repo (`private-gpt-fork`)

```
.brain-config.yaml                                    # NEW, committed
...
```
.gitignore     # add: project-brain/ AND .claude/skills/_brain_helpers/.venv/
```

### Files generated at runtime (gitignored)

```
project-brain/                          # gitignored, generated by /refresh-brain
project-brain/_INDEX.md                 # Claude-written orientation
project-brain/<concept-slug>.md         # Claude-written code-concept notes
project-brain/<NodeName>.md             # graphify-extracted node notes
project-brain/_COMMUNITY_*.md           # graphify-written community overviews
project-brain/graph.canvas              # graphify-written Obsidian graph
project-brain/graphify-out/             # graphify internal state
project-brain/.history/                 # backup of overwritten notes (strategy Z)
project-brain/.last-refresh-diff.md     # post-refresh diff log
```

### File responsibilities

- **`config.py`** — parse `.brain-config.yaml`, validate min schema (`source_paths`, `vault_dir`), resolve relative paths against repo root
...
name = "brain-helpers"
version = "0.1.0"
description = "Helpers for project-brain Claude Code skills"
...
**Files:**
- Create: `.brain-config.yaml`

### Task G1: Create config for this repo

- [ ] **Step 1: Write `.brain-config.yaml`**

```yaml
# Per-project config for project-brain
...
Expected:
```
source_paths: ['private_gpt/']
vault_dir: project-brain/
resolved sources: [PosixPath('/Users/.../private-gpt-fork/private_gpt')]
```
...
git commit -m "feat(brain): add .brain-config.yaml for this repo"
```
...
```bash
ls project-brain/ | head -20
ls project-brain/graphify-out/ 2>/dev/null
test -f project-brain/_INDEX.md && echo "INDEX exists"
test -f project-brain/graph.canvas && echo "Canvas exists"
```
...
```bash
head -20 project-brain/_INDEX.md
```
...
Open Obsidian → File → Open Vault → Browse → select `project-brain/` folder. Verify:
- Notes are readable
...
You should NOT immediately glob-read `private_gpt/components/retrieval/*.py`. If you find yourself wanting to, that's a signal that the concept notes are too vague — go back and rewrite them in next refresh.
...
- [ ] **Step 1: Update `docs/project-brain-instruction.md` status line**
...
```bash
git add docs/project-brain-instruction.md
...
- [ ] `.gitignore` correctly ignores `project-brain/` (run `git status` after refresh; no untracked files in vault)
...
- Multi-project portability (only tested on `private-gpt-fork`)
...
|------|-----------|-----------|
| graphify fails on `private_gpt/` (large codebase, semantic subagents time out) | Medium | Reduce scope to one subdirectory first (e.g., `private_gpt/components/retrieval/`); validate, then expand |
...
| `.brain-config.yaml` parsing edge cases | Low | Tests cover common cases; if user hits something exotic, add test + fix |
